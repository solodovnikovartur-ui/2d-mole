...
function isHintDot(col: number, row: number, state: GameState): boolean {
  const dot = state.hintDot;
  return dot !== null && dot.x === col && dot.y === row;
}
    ...
    <template v-for="(row, rowIndex) in state.blocks" :key="`row-${rowIndex}`">
      <div
        ...
        :class="[
          ...
          { 'cell--hint-trail': isHintDot(colIndex, rowIndex, state) },
        ]"
      />
    </template>
  </div>
